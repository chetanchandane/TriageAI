...
## Sprint 6 Patch: Two-Stage Emergency Confirmation (Mar 2026)

- **Problem — false positive EMERGENCY classification:** The safety agent was classifying routine or chronic symptom messages as EMERGENCY, short-circuiting the graph before the triage agent could apply any context. Examples: "I had a seizure last year and need to refill my medication", "I've had back pain for months", "mild shortness of breath when climbing stairs". Root causes: (1) rule match alone was sufficient to set `is_emergency=True` with no confirmation step; (2) the LLM fallback prompt included "when in doubt, answer true" which defeated the negative constraints for borderline cases; (3) LLM API failures silently defaulted to `is_potential_emergency=True`.
- **Fix — two-stage confirmation:** Rule fires → LLM confirmation call with a focused "is this ACTIVE and ACUTE right now?" prompt → only if both agree is `is_emergency=True` set. Single-layer matches (`rules_only`, `llm_only`) set `is_potential_emergency=True` but route through the triage agent, which uses patient history and policy tools to make a context-informed urgency decision. LLM failure fallback changed to `False` (rules already ran). Removed the "when in doubt, answer true" bias from the general LLM scan prompt; replaced with "when genuinely uncertain, answer false — the triage agent will assess further."
- **0% false negatives preserved:** All suspicious cases still surface `is_potential_emergency=True` in state. The triage agent sees the safety warning and will weight urgency accordingly. Nothing is silently cleared.
